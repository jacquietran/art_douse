...
# Gradient colours
gradient_colours <- c("#A80874", "#B7FDFE", "#E94F37", "#2B9720", "#E3C0D3")
...
# set.seed(initial_seed)
line_size <- sample(seq(0.01, 0.1, by = 0.01), 3)
# line_size <- 0.1
...
p <- ggplot2::ggplot() +
  ...
  ggplot2::geom_density_2d(
    data = data |> dplyr::filter(grouping_var == 6),
    ggplot2::aes(x = x, y = y, group = grouping_var),
    n = plot_res, colour = contour_col, adjust = adjust_factor, size = line_size[2]) +
  ggplot2::geom_density_2d(
    data = data |> dplyr::filter(grouping_var == 7),
    ggplot2::aes(x = x, y = y, group = grouping_var),
    n = plot_res, colour = contour_col, adjust = adjust_factor, size = line_size[3]) +
